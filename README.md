# Kobo-Reader Build Environment fork with fixes

> [!IMPORTANT]
> Includes legacy source packages, toolchains, patches, and build scripts required to compile the development environment for Kobo Readers and Kobo Desktop applications on nowadays Debian 13.
...
1. **Environment Setup**
-   Initialise and configure your local toolchain, sysroot (`fs`), temp directories and theb `build-config-user.sh` by sourcing the setup script:

```bash
source ./setup-allv.sh

```

2. **Compilation**
Once initialised, trigger the main build process:
```bash
./build/build-all.sh
...
For detailed information regarding legacy fixes, toolchain compatibility, and architecture patches, refer to the included guides:

* **[FORK-FIXES-NOTES.md](./FORK-FIXES-NOTES.md)**
